...
| 高级备份 | 已开通用户可直接创建服务端凭据任务，客户端只调用高级备份公开 API |
...
1. 打开点备客户端。
2. 在“登录”页面登录点备账号。旧在线备份和书架读取仍需要完成起点账号登录。
3. 在“搜索书籍”或“书架”页面选择书籍。
4. 使用“在线备份”创建任务并下载结果。
5. 已开通高级备份时，可在书籍详情页选择章节后点击“高级备份”，再到“高级备份”页面查看任务和下载 archive。
6. 如需备份手机本地缓存，连接 Android 设备后使用“本地备份”。

## 高级备份

高级备份用于服务端已配置凭据的场景。客户端不提供起点账号输入，不提交起点账号密码、Cookie 或签名参数，也不实现解密、签名或逆向算法。

客户端只调用 `/api/v1/advanced-backup/*`：

1. 在书籍详情页勾选章节并点击“高级备份”。
2. 客户端提交书籍 ID、书名、章节 ID、章节名和合并选项。
3. 在“高级备份”页面刷新任务列表或状态。
4. 任务完成后下载 archive，并在本地安全解压。
